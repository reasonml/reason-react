...
let makeStaticJsx ~loc  lst =
  ...
  | [] ->
      Exp.construct ~loc
        { loc; txt = Ldot(Lident "React", "Empty")} None
  | hd::[] -> hd
  | hd::hdHd::[] ->
  ...
  | hd::hdHd::hdHdHd::hdHdHdHd::tl ->
      Exp.construct ~loc { loc; txt = Ldot(Lident "React", "ElementMap")} (Some (Exp.tuple [hd; hdHd; hdHdHd; hdHdHdHd]))
...
let transformChildren ~loc ~mapper theList =
  ...
  | Some _ -> raise (Invalid_argument "JSX: the JSX version must be either 2, 3, or 4")
...
let jsxMapper () =
  ...
  let jsxTransformV4 modulePath mapper loc attrs callExpression callArguments =
