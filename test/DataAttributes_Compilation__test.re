...
describe("Data Attributes - PPX Compilation Tests", () => {
  describe("Basic data attribute compilation", () => {
    test("should compile data attributes on various element types with proper transformations", () => {
      let container = ReactTestingLibrary.render(
        <div>
          <div data_testid="single-test" />
          <span data_testid="multi-test" data_role="content" data_value="example" />
          <button data_testid="button-test" data_analytics="click-tracking" />
          <input data_testid="input-test" data_field="user-input" />
          <header data_testid="header-test" data_section="navigation" />
          <div data_testid="kebab-test" data_test_id="underscore" data_complex_name="camelCase" />
        </div>
      );
      
      // Test single attribute
      let singleElement = getByTestId("single-test", container);
      expect(singleElement->getAttribute("data-testid"))->toEqual(Some("single-test"));
      
      // Test multiple attributes
      let multiElement = getByTestId("multi-test", container);
      expect(multiElement->getAttribute("data-testid"))->toEqual(Some("multi-test"));
      expect(multiElement->getAttribute("data-role"))->toEqual(Some("content"));
      expect(multiElement->getAttribute("data-value"))->toEqual(Some("example"));
      ...
      expect(buttonElement->getAttribute("data-analytics"))->toEqual(Some("click-tracking"));
      expect(inputElement->getAttribute("data-field"))->toEqual(Some("user-input"));
      expect(headerElement->getAttribute("data-section"))->toEqual(Some("navigation"));
      
      // Test underscore to kebab-case transformation
      let kebabElement = getByTestId("kebab-test", container);
      expect(kebabElement->getAttribute("data-test-id"))->toEqual(Some("underscore"));
      expect(kebabElement->getAttribute("data-complex-name"))->toEqual(Some("camelCase"));
    });
    ...
    test("should compile data attributes alongside standard props", () => {
      ...
      expect(element->getAttribute("data-component"))->toEqual(Some("test-component"));
  ...
  describe("Dynamic values and expressions", () => {
    test("should handle dynamic values, expressions, and string operations", () => {
      ...
      let container = ReactTestingLibrary.render(
        <div>
          <div data_testid={testId} data_category={category} data_priority="high">
      ...
      expect(dynamicElement->getAttribute("data-testid"))->toEqual(Some("dynamic-test"));
      expect(dynamicElement->getAttribute("data-category"))->toEqual(Some("user-action"));
      expect(dynamicElement->getAttribute("data-priority"))->toEqual(Some("high"));
      
      // Test expressions
      let expressionElement = getByTestId("user-123-profile", container);
      expect(expressionElement->getAttribute("data-testid"))->toEqual(Some("user-123-profile"));
      expect(expressionElement->getAttribute("data-status"))->toEqual(Some("active"));
      expect(expressionElement->getAttribute("data-count"))->toEqual(Some("42"));
    });
    ...
    test("should handle function results and pattern matching", () => {
      ...
      expect(functionElement->getAttribute("data-env"))->toEqual(Some("development"));
      expect(functionElement->getAttribute("data-score"))->toEqual(Some("42"));
      expect(functionElement->getAttribute("data-length"))->toEqual(Some("3"));
      
      // Test pattern matching
      let patternElement = getByTestId("pattern-matching", container);
      expect(patternElement->getAttribute("data-status"))->toEqual(Some("success"));
      expect(patternElement->getAttribute("data-message"))->toEqual(Some("Data loaded successfully"));
    });
  });
  ...
  describe("Nested modules and component compilation", () => {
    test("should compile data attributes in nested module contexts", () => {
      ...
      let container = ReactTestingLibrary.render(<OuterModule testProp="test-value" />);
      
      // Test outer module
      let outerElement = getByTestId("outer-module", container);
      expect(outerElement->getAttribute("data-component"))->toEqual(Some("nested-test"));
      
      // Test inner module
      let innerElement = getByTestId("inner-test", container);
      expect(innerElement->getAttribute("data-category"))->toEqual(Some("demo"));
      expect(innerElement->getAttribute("data-level"))->toEqual(Some("deeply-nested"));
      
      // Test nested content
      let contentElement = getByTestId("inner-test-content", container);
      expect(contentElement->getAttribute("data-role"))->toEqual(Some("inner-content"));
      
      // Test prop passing
      let moduleContent = getByTestId("module-content", container);
      expect(moduleContent->getAttribute("data-prop"))->toEqual(Some("test-value"));
    });

    test("should handle complex nested structures with mixed element types", () => {
      let container = ReactTestingLibrary.render(
        <main data_testid="complex-structure" data_component="demo-app">
          <header data_role="app-header" data_testid="app-header">
            <h1 data_element="app-title"> {React.string("Complex Structure Test")} </h1>
          </header>
          ...
          <footer data_role="app-footer" data_testid="app-footer">
            <p data_content="footer-text"> {React.string("Complex structure demo")} </p>
          </footer>
        </main>
      );
      ...
      expect(mainElement->getAttribute("data-component"))->toEqual(Some("demo-app"));
      expect(headerElement->getAttribute("data-role"))->toEqual(Some("app-header"));
      expect(formElement->getAttribute("data-validation"))->toEqual(Some("required"));
      expect(inputElement->getAttribute("data-field"))->toEqual(Some("user-data"));
      expect(buttonElement->getAttribute("data-action"))->toEqual(Some("form-submit"));
      expect(footerElement->getAttribute("data-role"))->toEqual(Some("app-footer"));
    });
  });
    ...
    test("should handle same data attributes used multiple times without conflicts", () => {
      ...
      let container = ReactTestingLibrary.render(
        <div data_testid="deduplication-test" data_category={sharedCategory}>
          <div data_testid="item-1" data_category={sharedCategory} data_index="1">
            {React.string("Item 1")}
          </div>
          <div data_testid="item-2" data_category={sharedCategory} data_index="2">
            {React.string("Item 2")}
          </div>
          <div data_testid="item-3" data_category={sharedCategory} data_index="3">
            {React.string("Item 3")}
          </div>
      ...
      expect(parentElement->getAttribute("data-category"))->toEqual(Some("test-category"));
      expect(item1->getAttribute("data-category"))->toEqual(Some("test-category"));
      expect(item2->getAttribute("data-category"))->toEqual(Some("test-category"));
      expect(item3->getAttribute("data-category"))->toEqual(Some("test-category"));
      expect(spanElement->getAttribute("data-category"))->toEqual(Some("test-category"));
