...
function createRootWithClassName(className) {
  var elements = document.getElementsByClassName(className);
  if (elements.length !== 0) {
    return ReactDom.unstable_createRoot(elements[0]);
  } else {
    throw [
          Caml_builtin_exceptions.invalid_argument,
          "ReactDOMRe.Unstable.createRootWithClassName: no element of class " + (className + " found in the HTML.")
        ];
  }
}
...
function createRootWithId(id) {
  ...
  if (match == null) {
    throw [
          Caml_builtin_exceptions.invalid_argument,
          "ReactDOMRe.Unstable.createRootWithId: no element of id " + (id + " found in the HTML.")
        ];
  } else {
    return ReactDom.unstable_createRoot(match);
  }
}
